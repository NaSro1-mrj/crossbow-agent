...
def detect_deepfake(image_path: str) -> str:
    """
    Detect if an image is a deepfake using NVIDIA's AI deepfake detection API.

    This tool analyzes images to determine if they have been manipulated using
    deepfake technology, AI-generated content, or other synthetic media techniques.

    Args:
        image_path: Path to the image file to analyze (supports PNG, JPG, JPEG)

    Returns:
        str: JSON string containing detection results with confidence scores

    Example:
        >>> detect_deepfake("/path/to/suspicious_image.png")
        {
            "is_deepfake": true,
            "confidence": 0.95,
            "manipulation_type": "face_swap",
            "details": "High probability of AI-generated or manipulated content"
        }

    Use cases:
        - Verify authenticity of profile pictures
        - Detect manipulated evidence in investigations
        - Identify AI-generated fake images
        - Validate media in security incidents
        - Check for synthetic media in social engineering attacks
    """
    load_dotenv()
    ...
    if not nvidia_api_key:
        return json.dumps({
            ...
            "message": "Please set NVIDIA_API_KEY in .env file"
        })
    ...
    if not os.path.exists(image_path):
        return json.dumps({
            ...
            "message": "Please provide a valid image path"
        })
    ...
    def upload_asset(path, desc):
        """Upload large image as asset to NVIDIA API"""
        ...
        payload = {"contentType": "image/png", "description": desc}

        try:
            response = requests.post(assets_url,
                                     headers=headers,
                                     json=payload,
                                     timeout=30)
    ...
    try:
        ...
        # Determine if we need to upload as asset (for large images)
        if len(image_b64) < 180_000:
            ...
            # Large image - upload as asset
            asset_id = upload_asset(image_path, "Deepfake Detection Input")
...
def search_exploit_db(keyword: str,
                      platform: str = None,
                      nb_results: int = 10) -> str:
    """
    Search Exploit Database (Exploit-DB) for exploits by keyword.

    This tool searches the comprehensive Exploit-DB database for known exploits,
    proof-of-concept code, and security vulnerabilities. Essential for finding
    exploitation techniques for discovered vulnerabilities.

    Args:
        keyword: Search term (e.g., "WordPress 5.0", "Gitlab 14.9", "nginx")
        platform: Filter by platform (e.g., "php", "ruby", "linux", "windows", "python")
        nb_results: Maximum number of results to return (default: 10)

    Returns:
        str: JSON string containing exploit information with IDs, descriptions, dates, links

    Example:
        >>> search_exploit_db("Gitlab 14.9", platform="ruby", nb_results=5)
        [
            {
                "id": "50888",
                "description": "Gitlab 14.9 - Authentication Bypass",
                "type": "webapps",
                "platform": "Ruby",
                "date": "2022-04-26",
                "verified": false,
                "author": "Greenwolf",
                "link": "https://www.exploit-db.com/exploits/50888"
            }
        ]

    Use cases:
        - Find exploits for discovered software versions
        - Research attack techniques for vulnerabilities
        - Get proof-of-concept code for testing
        - Identify available exploits during pentesting
    ...
    try:
        ...
        # Search Exploit-DB
        results = pyxploitdb.searchEDB(keyword,
                                       platform=platform,
                                       _print=False,
        ...
        if not results:
            return json.dumps({
                ...
                "platform": platform,
                "results_count": 0,
                "message": "No exploits found for the given criteria",
                "exploits": []
            })
        ...
        for exploit in results:
            exploit_info = {
                ...
                "verified":
                bool(exploit.verified),
                "port":
                exploit.port if exploit.port else None,
                "author":
                exploit.author,
                "link":
                exploit.link,
                "tags":
                exploit.tag_if_any if hasattr(exploit, 'tag_if_any') else []
            }
        ...
        return json.dumps({
            "error": "pyxploitdb not installed",
            "message": "Install with: pip install pyxploitdb"
        })
    except Exception as e:
        return json.dumps({
            "error": "Exploit search failed",
            "message": str(e),
            "query": keyword
        })


def search_exploit_by_cve(cve_id: str) -> str:
    """
    Search Exploit Database for exploits by CVE identifier.

    This tool searches for known exploits associated with a specific CVE
    (Common Vulnerabilities and Exposures) identifier. Critical for determining
    if a discovered vulnerability has public exploits available.

    Args:
        cve_id: CVE identifier (e.g., "CVE-2021-44228", "CVE-2023-12345")

    Returns:
        str: JSON string containing exploit information for the CVE

    Example:
        >>> search_exploit_by_cve("CVE-2006-1234")
        [
            {
                "id": "27423",
                "description": "DSCounter 1.2 - 'index.php' SQL Injection",
                "type": "webapps",
                "platform": "PHP",
                "date": "2006-03-14",
                "verified": true,
                ...
                "cve": "CVE-2006-1234",
                "link": "https://www.exploit-db.com/exploits/27423"
            }
        ]

    Use cases:
        - Check if a CVE has public exploits
        - Find proof-of-concept code for CVEs
        - Assess exploitability of vulnerabilities
        - Prioritize patching based on exploit availability
        - Validate security scanner findings
    """
    try:
        import pyxploitdb
        ...
        if not cve_id.upper().startswith('CVE-'):
            return json.dumps({
                ...
                "message": "CVE ID should be in format: CVE-YYYY-NNNNN",
                "example": "CVE-2021-44228"
            })

        # Search by CVE
        results = pyxploitdb.searchCVE(cve_id)
        ...
        if not results:
            return json.dumps({
                ...
                "cve": cve_id,
                "results_count": 0,
                "message": f"No public exploits found for {cve_id}",
                "note":
                "This doesn't mean the vulnerability isn't exploitable, just no public PoC on Exploit-DB",
                "exploits": []
            })
        ...
        return json.dumps(
            {
                ...
                exploits,
                "recommendation":
                "Review exploit code before using. Test in authorized environment only."
            },
            indent=2)
...
SECURITY_TOOLS = [
    ...
    show_shell_session,
    get_shell_history,
    remote_traffic_capture,
    # Code execution
    execute_python_code_tool,
    execute_safe_python_script,
    execute_code,
    # Analysis tools
    extract_printable_strings,
    decode_base64_string,
    decode_hex_bytes_to_ascii,
    bandit,
    semgrep,
    nuclei,
    # AI Detection tools
    ...
    search_exploit_db,
    search_exploit_by_cve,
    # CTF tools
    think_about_ctf,
    analyze_ctf_situation,
    write_ctf_findings,
    read_ctf_findings,
