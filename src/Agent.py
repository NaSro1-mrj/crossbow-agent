...
class SecurityAgentSystem:
        ...
        if self.use_mcp and self.mcp_servers:
            try:
                ...
                for url_server in url_servers:
                    try:
                        mcp_tool = MCPTools(url=url_server['url'],
                                            transport=url_server.get(
                                                'transport',
                                                'streamable-http'))
                        tools.append(mcp_tool)
                    except Exception as e:
                        print(
                            f"⚠️  Warning: Failed to initialize MCP server {url_server.get('name', 'unknown')}: {e}"
                        )
    ...
    def _create_all_agents(self):
        ...
        self.soc_agent = Agent(
            name="SOC Analyst",
            role=
            "Security Operations Center - Log Analysis and Attack Investigation",
            instructions=[SOC_AGENT_PROMPT],
            **agent_kwargs,
        )

        self.threat_intelligence_agent = Agent(
            name="Threat Intelligence Analyst",
            role=
            "Cyber Threat Intelligence - IOC Collection and Threat Actor Profiling",
            instructions=[THREAT_INTELLIGENCE_AGENT_PROMPT],
            **agent_kwargs,
        )
    ...
    def _create_security_team(self):
        ...
        team_kwargs = {
            ...
            True,  # CRITICAL: Always preserve conversation history
        }
    ...
    def run_assessment(self, task: str, stream: bool = True):
        ...
        self.security_team.print_response(task,
                                          stream=stream,
    ...
    def get_agent(self, agent_type: str):
        ...
        agents = {
            ...
            "soc": self.soc_agent,
            "threat_intel": self.threat_intelligence_agent,
            "ti": self.threat_intelligence_agent,
        }
